...
/**
 * Formats an ISO 8601 string into a user-friendly date and time.
 * e.g., '2025-11-11T09:00:00' -> 'Nov 11, 2025, 9:00 AM'
 */
...
/**
 * Formats an ISO 8601 string into YYYY-MM-DD for date input.
 */
...
/**
 * Renders a colored badge based on the appointment status.
 */
...
/**
 * A modal component for adding or editing appointments.
 */
const AppointmentModal = ({ mode, initialData, onClose, onSubmit }) => {
  ...
  const handleDateTimeChange = (e) => {
     const { name, value } = e.target;
     // Combine date and time inputs into a single ISO-ish string
     if (name === 'datePart') {
        ...
        // Append seconds for consistency, though time input only gives HH:mm
  ...
  // Extract date and time parts for the input fields
...
const AdminAppointments = () => {
  ...
  const todayDateString = getTodayDateString(); // e.g., '2025-11-11'
  ...
  // 3. Paginate the sorted appointments
  ...
  const handleSort = (key) => {
    ...
    if (sortConfig.key === key && sortConfig.direction === 'ascending') {
      direction = 'descending';
       ...
       direction = 'ascending'; // Cycle
    }
    setSortConfig({ key, direction });
  };
  
  const getSortIndicator = (key) => {
    if (sortConfig.key !== key) return '↕'; // Default icon
    if (sortConfig.direction === 'ascending') return '↑'; // Up arrow
    return '↓'; // Down arrow
  };
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [debouncedSearchTerm, statusFilter, showTodayOnly]);

  const openModal = (mode, appointment = null) => {
    const modalData = mode === 'edit' ? appointment : {
      // Default values for a new appointment
      customerName: '',
      customerEmail: '',
      phone: '',
      service: '',
  ...
  const handleModalSubmit = (formData) => {
    if (modalState.mode === 'add') {
      ...
      setAppointments(prev => 
        prev.map(appt => 
          appt.id === modalState.data.id 
            ? { ...appt, ...formData, price: parseFloat(formData.price) } 
            : appt
        )
      );
    }
  ...
  const handleMarkCompleted = (appointment) => {
    ...
    // Warn if marking a future appointment as complete
    if (apptDate > now && !window.confirm('This appointment is in the future. Are you sure you want to mark it as completed?')) {
      return;
    }
          ...
          <thead>
            <tr>
              ...
              <th 
                ...
                aria-sort={sortConfig.key === 'dateTime' ? sortConfig.direction : 'none'}
              >
                  ...
                  <td>
                    <div className="action-buttons">
                      ...
                      <button 
                        ...
                        aria-label={`Delete appointment for ${appt.customerName}`}
                      >
                        Delete
                      </button>
                    </div>
